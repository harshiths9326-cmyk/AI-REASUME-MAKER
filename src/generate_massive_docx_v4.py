...
def add_code(doc, code_text):
    for line in code_text.split('\n'):
        ...
        # keep it simple, one run per line
        run = p.add_run(line)
...
code_files_config = [
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\app\page.tsx",
        "title": "9.1 Landing Page Implementation (app/page.tsx)",
        "explanation": [
            "This is the entry point of the application. It utilizes Framer Motion for animations and serves as the marketing frontend.",
            "The code clearly demonstrates a separation between the marketing copy and the actual application logic, utilizing standard Next.js routing to direct the user to the builder via the 'Initialize Builder' call to action."
        ]
    },
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\app\builder\page.tsx",
        "title": "9.2 Main Builder Orchestrator (app/builder/page.tsx)",
        "explanation": [
            "This file represents the absolute core of the application's state management. It is a client component ('use client') that holds the master state tree for the ResumeData.",
            "The dual `useEffect` hooks here are critical: the first retrieves data from `sessionStorage` on initial load, and the second continuously writes data back to `sessionStorage` whenever the state mutates. This is what provides the application with its resilient, crash-proof user experience.",
            "The layout utilizes CSS Grid (`grid-cols-1 lg:grid-cols-2`) to create the signature split-pane design on desktop screens, gracefully falling back to a stacked layout on mobile devices."
        ]
    },
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\components\resume\resume-form.tsx",
        "title": "9.3 Master Form Component (resume-form.tsx)",
        "explanation": [
            "This component acts as the parent container for all the individual data entry modules (PersonalInfo, Experience, Education, etc.).",
            "It receives the master `data` object and the `updateData` dispatch function as props, and effectively drills these down to its children. This pattern ensures that any change in a sub-component immediately propagates up to the master orchestrator.",
            "It also includes the template switching logic in the sidebar, allowing users to dynamically swap out the visual presentation without losing their underlying data."
        ]
    },
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\components\resume\resume-preview.tsx",
        "title": "9.4 Real-Time Preview Engine (resume-preview.tsx)",
        "explanation": [
            "This is arguably the most complex rendering component in the system. It implements the 'Factory Pattern' to dynamically render different template components based on the user's selection.",
            "The `useReactToPrint` hook is heavily utilized here. We attach a `useRef` to the master wrapper `div`. When the user clicks the download button, the print engine targets this specific ref, completely ignoring the sidebar, buttons, and other UI elements.",
            "The component also includes logic to temporarily lock the UI (via a loading state) to ensure no data mutations occur while the browser is spooling the PDF document to memory."
        ]
    },
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\components\resume\personal-info.tsx",
        "title": "9.5 Personal Info Component (personal-info.tsx)",
        "explanation": [
            "This file showcases the implementation of a standard 'Controlled Component' in React. Every input field's `value` is directly tied to the React state.",
            "A unified `handleChange` function uses the `name` attribute of the HTML inputs to dynamically update the correct property in the state object. This minimizes the amount of code required to manage dozens of input fields."
        ]
    },
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\components\resume\experience.tsx",
        "title": "9.6 Experience Array Manager (experience.tsx)",
        "explanation": [
            "Unlike the Personal Info component, this component must manage an Array of objects, which is significantly more difficult in React. It includes logic to dynamically add, update, and remove array items.",
            "When adding a new experience, the component generates a cryptographically secure UUID (`crypto.randomUUID()`) to serve as the React `key`. This prevents catastrophic rendering bugs that occur when using array indices as keys during item deletion.",
            "The `updateExperience` function uses an immutable mapping pattern (`data.map(item => item.id === id ? ... : item)`) to ensure that React detects the state change and triggers a re-render."
        ]
    },
    {
        "path": r"c:\Users\Bhoomika\Desktop\nh.intern\src\lib\types.ts",
        "title": "9.7 TypeScript Data Models (types.ts)",
        "explanation": [
            "This file is the bedrock of the application's stability. By explicitly defining the shape of every single object in the system, we eliminate an entire class of runtime errors.",
            "These interfaces serve as the contract between the frontend form, the rendering engine, and the backend database. Any data that flows through the system must strictly adhere to these definitions."
        ]
    }
]
...
def main():
    ...
    # 4. Generate Content (Chapters 1-8)
    ...
    # 5. Generate Code Appendix (Chapter 9)
    add_heading(doc, "9. SOURCE CODE ANALYSIS & ARCHITECTURE", level=1)
    add_paragraph(doc, "The following sections represent the entire core codebase of the AI Resume Maker. The inclusion of this source code demonstrates the practical application of the theoretical concepts discussed in the preceding chapters. Each file is presented in its entirety, followed by a rigorous technical analysis of its functions, hooks, and architectural significance.")
    doc.add_page_break()
    
    for config in code_files_config:
        add_heading(doc, config['title'], level=2)
        
        # Add explanation paragraphs BEFORE the code
        for exp_text in config['explanation']:
            add_paragraph(doc, exp_text)
        ...
        try:
            ...
            add_paragraph(doc, f"[Error reading file: {config['path']} - {e}]")
            
        doc.add_page_break()

    output_path = r"C:\Users\Bhoomika\Desktop\AI_Resume_Maker_Final_Master_Report_V4.docx"
    doc.save(output_path)
    print(f"Hyper-Massive DOCX created successfully at: {output_path}")
